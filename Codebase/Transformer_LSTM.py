...
with strategy.scope():
    ...
    decoder_concat = keras.layers.Concatenate()([decoder_input_t1, decoder_input_t2])#, decoder_startstop])
    ...
    decoder_hidden_output = decoder_block([decoder_add, decoder_context_input])#([decoder_mask_pos_embedding, decoder_context_input])
...
# context = encoder_model.predict({"enc_token_1":t1_test[0:1], "enc_token_2":t2_test[0:1]})
# context.shape
...
# output = decoder_model.predict({"inner_enc_state": context, "dec_token_1": dec_t1, "dec_token_2": dec_t2, "dec_start/stop": dec_s_s})

# dec_t1[:,1:2,:] = output[0][:,0:1,:]
# dec_t2[:,1:2,:]  = output[1][:,0:1,:]
# dec_s_s[:,1:2,:] = np.round(output[2][:,0:1,:])
# output_2 = decoder_model.predict({"inner_enc_state": context, "dec_token_1": dec_t1, "dec_token_2": dec_t2, "dec_start/stop": dec_s_s})
